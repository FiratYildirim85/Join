...
function templateTask(i) {
   return /*html*/ `    
      <div id='${i}' draggable="true" ondragstart="startDragging(${i})" class="box" onclick="openTask(${i})">
         <span id="changeColorOfCategoryAfterDragAndDrop${i}" class="category">${toDosArray[i].category}</span>
         <span class="title">${toDosArray[i].title}</span>
         <span class="description">${toDosArray[i].description}</span>
...
function showAssignedTo(i) {
   for (let y = 0; y < allUsersAndContacts.length; y++) {
      if (allUsersAndContacts[y].second_name == toDosArray[i].assigned) {
...
function getSecondName(i, y) {
   for (let x = 0; x < allUsersAndContacts.length; x++) {
      if (allUsersAndContacts[x].second_name == toDosArray[i].assigned) {
...
function changeAssingnChangeTask(assignCount) {
   ...
   if (img.src == "assets/img/addTask_rectangle.png") {
      currentAssigned.splice(assignCount, 1, allUsersAndContacts[assignCount].second_name);
      img.src = "assets/img/checkbox.png";
   } else if (img.src == "assets/img/checkbox.png") {
